...
// clone메서드로 깊은복사가 가능하게 하려면 반드시 Cloneable 인터페이스를 구현하고 clone메서드를 오버라이딩해야 한다.
// 왜? Cloneable 인터페이스는 표시 인터페이스이다. 깊은 복사를 수행하는 클래스이다라고 표시만 합니다.
// clone()메서드는 protected멤버이므로 접근이 불가능하다. 그래서 접근 가능하도록 오버라이딩해야 한다.
@AllArgsConstructor
@NoArgsConstructor
@Data
...
public class CloneEx {
	public static void main(String[] args) {
		...
		try {
			...
			c4.setName("또 다른놈");
			System.out.println(c1 +  " : " + c4);  // 1개만 바뀜. 다른 객체다!!!
